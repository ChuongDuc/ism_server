...
import { smDb, sequelize } from '../../loader/mysql';
...
const user_resolver: IResolvers = {
    ...
    Query: {
        me: async (_parent, _, context: SmContext) => {
            ...
            const { user } = context;
            if (user?.id) {
                return await smDb.user.findByPk(user.id, {
        ...
        login: async (parent, { input }) => {
            ...
            const user = await smDb.user.findOne({
                where: {
                    [Op.or]: {
                        userName: account,
                        phoneNumber: account,
    ...
    Mutation: {
        createUser: async (parent, { input }, context: SmContext) => {
            checkAuthentication(context);
            if (
            ...
            const createdUser = await smDb.user.findOne({
                where: {
            ...
            return await sequelize.transaction(async (t: Transaction) => {
                try {
                    const newUser = await smDb.user.create(userAttribute, {
                        transaction: t,
                    });
